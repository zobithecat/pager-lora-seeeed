...
BAT+ ──[ R1 2.3M ]──┬──[ R2 2.3M ]── GND
                    ├── D5 (GPIO6)
                   ═╪═ C 100 nF ~ 1 µF → GND     ← 필수 (아래)
```

- R1 = R2면 아무 값이나 된다(100k–2.3M, 상시 소모 10 µA~1 µA). 비율이 다르면 `VEH_VBAT_DIVIDER = (R1+R2)/R2`
- **콘덴서가 필수인 이유**: ESP32 SAR ADC는 샘플링 순간 수 pF를 순간 충전하는데, 소스 임피던스가 1.15 MΩ이면 그 시간에 못 채워 값이 낮고 요동친다. D5–GND의 100 nF~1 µF가 샘플 전하를 대주고 저항이 천천히 되채운다(RC ≈ 0.1–1 s, 배터리 전압 변화보다 훨씬 빠름)
- 4.2 V → 핀에는 2.1 V. **BAT+를 GPIO에 직접 꽂으면 안 된다** (3.3 V 초과)
- 멀티미터로 잰 값과 다르면 `VEH_VBAT_CAL = 실측 / 표시값`
- `VEH_VBAT_ADC_PIN`은 `6`(D5)으로 켜져 있다. 2.5–4.5 V 밖의 값은 "측정 불가"(`-`)로 처리하므로 배선 전에도 무해
